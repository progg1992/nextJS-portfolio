...
const Modal = ({ onClose, toggle }) => {
  ...
  useEffect(() => {
    let modalElement = document.getElementById("my-modal");
    if (!modalElement) {
      modalElement = document.createElement("div");
      modalElement.id = "my-modal";
      document.body.appendChild(modalElement); // Dynamically create the modal container if it doesn't exist
    }
    setModalContainer(modalElement);

    return () => {
      if (modalElement && document.body.contains(modalElement)) {
        document.body.removeChild(modalElement); // Clean up modal on component unmount
      }
    };
  }, []);

  if (!modalContainer) return null; // Render nothing until modal container is available
  ...
  return (
    <div className="fixed top-4 right-2.5 xs:right-4 z-50 group">
      ...
      <audio ref={audioRef} loop>
        <source src={"/audio/techno-background.mp3"} type="audio/mpeg" />
